function Get-FlexaDocAssets {
	...
	if ($id -and $CompanyId) {
		$Asset = Invoke-FlexaDocRequest -Method get -Resource "/api/v1/companies/$CompanyId/assets/$Id"
		return $Asset
	} else {
		...
		$i = 1;
		$AllAssets = do {
			$Assets = Invoke-FlexaDocRequest -Method get -Resource "/api/v1/assets?page=$i&page_size=1000$($ResourceFilter)"
			$i++
			$Assets.Assets
