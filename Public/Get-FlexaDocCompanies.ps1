function Get-FlexaDocCompanies {
	[CmdletBinding()]
	Param (
		[String]$Name = '',
		[Alias("phone_number")]
	...
	if ($Id) {
		$Company = Invoke-FlexaDocRequest -Method get -Resource "/api/v1/companies/$Id"
		return $Company
	} else {
		...
		$i = 1;
		$AllCompanies = do {
			$Companies = Invoke-FlexaDocRequest -Method get -Resource "/api/v1/companies?page=$i&page_size=1000$($ResourceFilter)"
			$i++
			$Companies.Companies
